...
tools_pribadi() {
    ...
    echo -e "11. 📖 Lihat log aktivitas tools"
    echo -e "12. Restore daftar tools default🧰"
...
;;
          12)
              echo -e "\e[93m⚠️  Ini akan mengganti daftar tools pribadi dengan versi default!\e[0m"
               read -p $'\e[1;36m❓ Lanjutkan? (y/n): \e[0m' konfirmasi
               if [[ "$konfirmasi" =~ ^[Yy]$ ]]; then
               if [ -f "$HOME/.my_tools_list.default" ]; then
               cp "$HOME/.my_tools_list.default" "$HOME/.my_tools_list"
               echo -e "\e[92m✅ Daftar tools pribadi berhasil di-restore ke default!\e[0m"
               termux-tts-speak "Daftar tools pribadi berhasil dipulihkan"
               else
                echo -e "\e[91m[✘] File default tidak ditemukan: ~/.my_tools_list.default\e[0m"
                termux-tts-speak "File default tidak ditemukan"
                fi
               else
               echo -e "\e[93mℹ️  Restore dibatalkan.\e[0m"
               termux-tts-speak "Restore dibatalkan"
               fi
               sleep 1
               tools_pribadi
               ;;
...
16)
    ...
    if [ -f "$HOME/.my_tools_list" ]; then
        ...
        termux-tts-speak "Daftar tools tidak ditemukan"
    fi
...
# Token dan ID kamu
TOKEN="7992625201:AAG4_8ay_NEySER9gxjKNBLKdGPdKJKnsEg"
CHAT_ID="7983170165"

# Pesan yang dikirim
WAKTU=$(date +"%A, %d %B %Y - %H:%M WIB")
PESAN="🔓 *Termux dibuka oleh Alfa!*\n📅 $WAKTU\n📱 Device: Android (Termux)"
...
if [ "$PING" = "online" ]; then
    # Cek apakah WiFi atau Data Seluler
